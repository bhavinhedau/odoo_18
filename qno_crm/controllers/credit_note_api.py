...
class SAPCreditAPI(http.Controller):

    def _get_valid_api_key(self):
        return request.env['ir.config_parameter'].sudo().get_param('sap.odoo.api.key')

    @http.route('/api/credit', type='json', auth='none', methods=['POST'], csrf=False)
    def sync_credit(self, **kwargs):
        current_credit = None
        current_line = None
        current_batch = None
        ...
        try:
            ...
            Credit = request.env['sap.credit'].sudo()
            Line = request.env['sap.credit.line'].sudo()
            Batch = request.env['sap.credit.batch.line'].sudo()

            existing = Credit.search([('doc_entry', '=', doc_entry)], limit=1)
            current_credit = doc_entry
            ...
            if existing:
                credit = existing
                credit.write(credit_vals)
                message = "Updated successfully"
            else:
                credit = Credit.create(credit_vals)
                message = "Created successfully"

            incoming_line_nums = {l.get("LineNum") for l in payload.get("Lines", [])}
            for old_line in credit.line_ids:
                if old_line.line_num not in incoming_line_nums:
                    old_line.unlink()

            existing_lines = {line.line_num: line for line in credit.line_ids}
            ...
            return {"success": True, "message": message, "doc_entry": credit.doc_entry}

        except Exception as e:
            return {
                "success": False,
                "message": "Error during credit note processing",
                "doc_entry": current_credit,
                "line_num": current_line,
