...
class MachineryProgress(models.Model):
    ...
    opening_petrol = fields.Float(string='Opening Petrol', compute="_compute_opening_fuel", store=True)
    opening_diesel = fields.Float(string='Opening Diesel', compute="_compute_opening_fuel", store=True)
    closing_petrol = fields.Float(string='Closing Petrol', compute="_compute_closing_fuel")
    closing_diesel = fields.Float(string='Closing Diesel', compute="_compute_closing_fuel")
    petrol_received = fields.Float(string='Petrol Received', digits=(16, 2))
    diesel_received = fields.Float(string='Diesel Received', digits=(16, 2))
    ...
    def _check_total_hours(self):
        for rec in self:
            for line in rec.progress_line_ids:
                # total = 0
                # print(line.working_hours,line.breakdown_hours,line.idle_hours)
                # total = (abs(line.working_hours) or 0)
                # total += (abs(line.breakdown_hours) or 0)
                # total += (abs(line.idle_hours) or 0)
                # print(total)
                # if total > 24.00:
                #     raise ValidationError(
                #         f"Equipment {line.equipment_id.display_name}: "
                #         "Total hours (Working + Breakdown + Idle) cannot exceed 24 hours."
                #     )
                ...
                if line.closing_reading_hr > 0.0 and line.closing_reading_hr < line.opening_reading_hr:
                    raise ValidationError(
                        f"Equipment {line.equipment_id.display_name}: "
...
class MachineryProgressLine(models.Model):
    ...
    project_id = fields.Many2one("project.master", string="Project Code", related='progress_id.project_id')
    state = fields.Selection(
    ...
    purpose = fields.Many2one('equipment.purpose', string="Equipment Purpose",
                              domain="[('id', 'in', available_purpose_ids)]")
    ...
    opening_reading_hr = fields.Float(string='HR Opening Reading', compute='_compute_opening_reading', default=0,
                                      store=True)
    closing_reading_km = fields.Float(string='KM Closing Reading')
    closing_reading_hr = fields.Float(string='HR Closing Reading')
    ...
    @api.constrains('working_hours', 'breakdown_hours', 'idle_hours')
    def _check_total_hours(self):
        for rec in self:
            ...
            if rec.progress_id.state in ('draft'):
                if (abs(rec.working_hours)) + (abs(rec.breakdown_hours)) + (abs(rec.idle_hours)) > 24:
                    raise ValidationError("Total hours (working + breakdown + idle) cannot exceed 24 hours.")
    ...
    # on change input values distroy
    @api.onchange('equipment_id', 'equipment_condition')
    def _onchange_equipment_config(self):
        print("_onchange_equipment_config")
        for rec in self:
            print(rec.closing_reading_km)
    ...
    @api.onchange('equipment_condition')
    def _onchange_equi_condition(self):
        for rec in self:
